...
class ERC_model(nn.Module):
    def __init__(self, model_type, clsNum, last, freeze, initial):
        ...
        if 'roberta' in model_type:
            self.context_model = RobertaModel.from_pretrained(model_path, ignore_mismatched_sizes=True)
                    
            if initial == 'scratch':
                config = RobertaConfig.from_pretrained(model_path, ignore_mismatched_sizes=True)
                self.speaker_model = RobertaModel(config)
            else: # pretrained
                self.speaker_model = RobertaModel.from_pretrained(model_path, ignore_mismatched_sizes=True)
