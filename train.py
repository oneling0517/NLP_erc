...
#     save_path = os.path.join(dataset+'_models', model_type, initial, freeze_type, dataclass, str(sample))
    ...
    for epoch in tqdm(range(training_epochs)):
        ...
        for i_batch, data in enumerate(train_dataloader):
            if (i_batch+1) % print_per_iteration == 0:    # print every 2000 mini-batches
                print(f'[ep {epoch + 1}][{i_batch + 1:5d}/{len(train_dataloader):5d}]')
...
#         logger.info('Final Fscore ## test-accuracy: {}, test-fscore: {}, test_epoch: {}'.format(test_acc, test_fbeta, best_epoch))
        logger.info('Final Fscore ## test-accuracy: {}, dev-fscore: {}, best_dev_epoch: {}'.format(dev_acc, dev_fbeta, best_epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument( "--epoch", type=int, help = 'training epohcs', default = 6) # 12 for iemocap
