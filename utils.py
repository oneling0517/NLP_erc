...
def make_batch_roberta(sessions):
    ...
    for session in sessions:
        ...
        for turn, (speaker, utt) in enumerate(zip(context_speaker, context)):
            ...
            #print("inputString speaker:", inputString)
            inputString += utt + " "
            ...
            if turn<len(context_speaker)-1 and speaker == now_speaker:
                ...
                #print("if turn<len(context_speaker)-1 and speaker == now_speaker, speaker_utt_list = ", speaker_utt_list)
...
# +
def make_batch_roberta_test(sessions):
    batch_input, batch_labels, batch_speaker_tokens = [], [], []
    for session in sessions:
        data = session[0]
        label_list = session[1]
        ...
        context, context_speaker = data
        now_speaker = context_speaker[-1]
        speaker_utt_list = []
        
        
        inputString = ""
        for turn, (speaker, utt) in enumerate(zip(context, context_speaker)):
